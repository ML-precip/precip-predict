...
class DeepFactory_Keras(Layer):
    ...
    def build_Davenport_2021(self):
        ...
        x = keras.layers.Conv2D(16, 3, padding='same', activity_regularizer=regularizers.l2(0.01))(inputs)
        x = keras.layers.Activation('relu')(x)
        x = keras.layers.MaxPooling2D(pool_size=2)(x)
        x = keras.layers.SpatialDropout2D(self.dropout_rate)(x) # In original: simple Dropout
        x = keras.layers.Conv2D(16, 3, padding='same', activity_regularizer=regularizers.l2(0.01))(x)
        x = keras.layers.Activation('relu')(x)
        x = keras.layers.MaxPooling2D(pool_size=2)(x)
        x = keras.layers.SpatialDropout2D(self.dropout_rate)(x) # In original: simple Dropout
        x = keras.layers.Flatten()(x)                
        x = keras.layers.Dense(self.latent_dim, activity_regularizer=regularizers.l2(0.001))(x) # In original: 16
        x = keras.layers.Activation('relu')(x)
        ...
        x = keras.layers.Reshape(target_shape=next_shape)(x)
    ...
    def build_Pan_2019(self):
        ...
        # Downsampling
        inputs = keras.layers.Input(shape=self.input_size)
        x = keras.layers.Conv2D(15, 4, padding='same')(inputs)
        x = keras.layers.Activation('relu')(x)
        x = keras.layers.BatchNormalization()(x)
        x = keras.layers.Conv2D(20, 4, padding='same')(x)
        x = keras.layers.Activation('relu')(x)
        x = keras.layers.BatchNormalization()(x)
        x = keras.layers.MaxPooling2D(pool_size=2)(x)
        x = keras.layers.Conv2D(20, 4, padding='same')(x)
        ...
        x = keras.layers.Dense(self.latent_dim, activation='relu')(x) # In original: 60
    ...
    def build_convLTSM(self):
        ...
        x = keras.layers.Reshape(target_shape=expanded_shape)(inputs)
        
        x = keras.layers.ConvLSTM2D(filters=32, kernel_size=(3, 3), return_sequences=True, padding = 'same',
                              go_backwards=True, activation='tanh', data_format = 'channels_last',
                              dropout=0.4, recurrent_dropout=0.2)(x)
        x = keras.layers.BatchNormalization()(x)

        x = keras.layers.ConvLSTM2D(filters=16, kernel_size=(3, 3), return_sequences=True, padding = 'same',
                              go_backwards=True, activation='tanh', data_format = 'channels_last',
                              dropout=0.4, recurrent_dropout=0.2)(x)
        x = keras.layers.BatchNormalization()(x)

        x = keras.layers.ConvLSTM2D(filters=8, kernel_size=(3, 3), return_sequences=False, padding = 'same',
                              go_backwards=True, activation='tanh', data_format = 'channels_last',
                              dropout=0.3, recurrent_dropout=0.2)(x)
        x = keras.layers.BatchNormalization()(x)

        x = keras.layers.Conv2D(filters=16, kernel_size=(1, 1), activation='relu', data_format='channels_last')(x)
        ...
        x = keras.layers.Conv2DTranspose(filters=1, kernel_size=(1, 1), strides=(2, 2),  activation='sigmoid',
    ...
    def conv_block(self, input, filters, kernel_size=3, stride=1, padding='same', initializer='default', activation='default', 
        ...
        if initializer == 'default':
            x = keras.layers.Conv2D(filters, kernel_size, strides=(stride, stride), padding=padding, activation=conv_activation)(input)
        else:
            x = keras.layers.Conv2D(filters, kernel_size, strides=(stride, stride), padding=padding, activation=conv_activation, kernel_initializer=initializer)(input)
    ...
    def deconv_block(self, input, filters, kernel_size=3, stride=1, padding='same', initializer='default', activation='default', 
        ...
        if self.unet_use_upsample:
            x = keras.layers.UpSampling2D((2, 2))(input)
        else:
            if initializer == 'default':
                x = keras.layers.Conv2DTranspose(filters, kernel_size, strides=stride, padding=padding, activation=activation)(input)
            else:
                x = keras.layers.Conv2DTranspose(filters, kernel_size, strides=stride, padding=padding, activation=activation, kernel_initializer=initializer)(input)
    ...
    def dense_block(self, input, units, activation='default', with_dropout=False):
        ...
        x = keras.layers.Dense(units, activation=activation)(input)
        if with_dropout:
    ...
    def padding_block(self, x, factor):
        ...
        if dh > 0 or dw > 0:
            ...
            left_pad = dw//2
            right_pad = dw//2 + dw%2
            x = keras.layers.ZeroPadding2D(padding=((top_pad, bottom_pad), (left_pad, right_pad)))(x)
    ...
    def final_cropping_block(self, x):
        ...
        x = keras.layers.Cropping2D(cropping=((dh//2, dh-dh//2), (dw//2, dw-dw//2)))(x)
